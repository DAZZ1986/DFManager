...
## 1. Определение целей тестирования
	...
	- На каком уровне? (бизнес-логика или системные требования)
	- Какие риски покрываем? (основной функционал, ошибки, edge cases)

## 2. Группировка тест-кейсов
	### A. По видам тестов			(Unit, API, UI)
	### B. По уровням тестирования	(бизнес, система)
	### C. По категориям сценариев	(позитив, негатив)

## 3. Алгоритм составления тест-кейсов
	### Шаг 1. Разделите тесты по видам
		Вид: Integration, API
	### Шаг 2. Определите уровень
		Бизнес-уровень
	### Шаг 3. Покройте все категории сценариев
		Для каждой функции/эндпоинта/формы:
...
## Вывод: как систематизировать процесс
	...
	4. Приоритезируйте: сначала основные сценарии, потом edge cases.
...
Категории:
	1. "Основной позитивный случай" - Успешный трансфер:
	Шаг																ТД								Ожидаемый результат
	- отправлен POST запрос с фронта								куплен игрок с id 6				отправлен POST запрос с заполненным в параметрах урл: clubId, playerId.
	...
	- трансфер модуля вычитал сообщение из RabbitMQ													в консоли наблюдаем сообщениме "RabbitMQ_Json_Consumer:  Received JSON message -> ...".
	- смена id клуба у игрока																		в таблице Player у записи с id 6, заполнилось поле clubId, id клубом из шага 1.
	- у клуба были вычтены деньги за игрока															в таблице Club у записи клуба бюджет клуба уменьшился на стоимость игрока.

	2. "Негативный с валидными данными" - Не успешный трансфер:
	Шаг																ТД								Ожидаемый результат
	- отправлен POST запрос с фронта								куплен игрок с id 6				отправлен POST запрос с заполненным в параметрах урл: clubId, playerId.
	- НЕ отправлен POST запрос с трансфер модуля в брокер модуль	json объект						в консоли наблюдаем сообщениме "Transfer failed! (money not enough)".
...
_______________________________________________________________________________________________________________________
		...
		Тест-кейсы:
			...
			2. Покупка игрока клубом с НЕ достаточным кол-вом денег.

			Негативные:
			1. Попытка передать существующего игрока в несуществующий клуб.
			2. Попыа передать несуществующего игрока в существующий клуб.

		Тест-кейсы интеграционные (тестирование API):
			Позитивные:
			1. Проверить ветвляющее поле в json с true
			2. Проверить ветвляющее поле в json с false
		 */
